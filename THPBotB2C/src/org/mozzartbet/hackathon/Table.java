...
import org.mozzartbet.hackathon.util.PokerActTask;
...
public class Table {
    ...
    /** observer for state tracking **/
    private IActObserver actObserver;
     ...
     *            The size of the big blind.
     */
    public Table(TableType type, int bigBlind, IActObserver actObserver) {
        this.tableType = type;
        this.bigBlind = bigBlind;
        this.actObserver = actObserver;
    ...
    public void run() {
        for (Player player : players) {
            player.getBot().joinedTable(bigBlind);
        }
        actObserver.joinedTable(tableType, bigBlind, players);
        dealerPosition = -1;
        actorPosition = -1;
        while (true) {
        ...
        notifyMessage("Kraj igre. Pobednik %s" + findWinner(players));
    }

    private String findWinner(List<Player> players) {
        String playerWinner = "";
    ...
    private void resetHand() {
        ...
        for (Player player : players) {
            player.getBot().handStarted(dealer);
        }
        actObserver.handStarted(dealer);
        notifyPlayersUpdated(false);
        notifyMessage("Nova ruka, %s deli karte.", dealer);
    }
    ...
    private void rotateActor() {
        ...
        actor = activePlayers.get(actorPosition);
        for (Player player : players) {
            player.getBot().actorRotated(actor);
            actObserver.actorRotated(actor);
        }
    }
    ...
    private void dealHoleCards() {
        ...
        notifyPlayersUpdated(false);
        notifyMessage("%s deli sve karte.", dealer);
    }
    ...
    private void dealCommunityCards(String phaseName, int noOfCards) {
        ...
        notifyPlayersUpdated(false);
        notifyMessage("%s deli %s.", dealer, phaseName);
    }
        ...
        while (playersToAct > 0) {
            ...
            if (actor.isAllIn()) {
                ...
                try {
					...
					Future<Action> result = EXECUTOR_SERVICE.submit(new PokerActTask(bet, minBet, actor, allowedActions));
					action = result.get(10, TimeUnit.SECONDS);
					...
					if (action == Action.CHECK) {
					    ...
					    if (amount > actor.getCash() && actor.getCash() >= minBet) {
					        System.out.println(amount + " je ulozeno. Igrac " + actor.getName() + " Stanje " + actor.getCash());
					    ...
					    playersToAct = doFold(playersToAct);
					} else {
	...
	private int doFold(int playersToAct) {
		actor.setCards(null);
		...
		if (activePlayers.size() == 1) {
		    ...
		    winner.win(amount);
		    notifyBoardUpdated();
		    notifyMessage("%s pobedjuje $ %d.", winner, amount);
		    return 0;
		}
		return playersToAct;
	}
...
//        System.out.format("[DEBUG]  Total: %d\n", getTotalPot());
        ...
        for (Player playerToShow : showingPlayers) {
            ...
            if (doShow) {
                // Show hand.
                for (Player player : players) {
                    player.getBot().playerUpdated(playerToShow);
                    actObserver.playerUpdated(playerToShow);
                }
                notifyMessage("%s ima %s.", playerToShow, handValue.getDescription());
            } else {
                // Fold.
                ...
                for (Player player : players) {
                    if (player.equals(playerToShow)) {
                        ...
                        player.getBot().playerUpdated(playerToShow.publicClone());
                        actObserver.playerUpdated(playerToShow);
                    }
                }
                notifyMessage("%s odustaje.", playerToShow);
            }
        }
...
//                                System.out.format("[DEBUG] %s receives an odd chip from the pot.\n", winner);
        ...
        for (Player winner : potDivision.keySet()) {
            ...
            winnerText.append(String.format("%s osvaja $ %d", winner, potShare));
    ...
    private void notifyMessage(String message, Object... args) {
        ...
        actObserver.messageReceived(message);
    ...
    private void notifyBoardUpdated() {
        int pot = getTotalPot();
        for (Player player : players) {
            player.getBot().boardUpdated(board, bet, pot);
        }
        actObserver.boardUpdated(board, bet, pot);
    }
    ...
    private void notifyPlayersUpdated(boolean showdown) {
        for (Player playerToNotify : players) {
            for (Player player : players) {
                actObserver.playerUpdated(player);
                if (!showdown && !player.equals(playerToNotify)) {
                    // Hide secret information to other players.
                    player = player.publicClone();
                }
                playerToNotify.getBot().playerUpdated(player);
    ...
    private void notifyPlayerActed() {
        for (Player p : players) {
            ...
            p.getBot().playerActed(playerInfo);
